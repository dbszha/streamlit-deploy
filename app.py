...
upper_bound = Q3 + 1.5 *+ IQR # 이상치 하한값
...
# stepS: 데이터셋 분할(train-test split)
...
# step6: 머신러닝 모델 학습
# 우리가 사용하는 모델은 랜덤 포레스트 모델입니다.
# 랜덤 포레스트는 결정 트리를 여러개 조합하여 예측 정확도를 높이는 모델입니다.
...
# recall(재현율): 실제 비만 중에서 모델이 맞춘 비율 => 비만인 사람을 놓치지 않고 얼마나 예측했는가?
#F1-score: Precision과 Recall의 조화평균=>모델의 전체적인 성능을 나타냄
# support 각 클래스(label)에 속하는 샘플의 개수 => 데이터셋 내에 존재하는 해당 클래스 개수
